...
void VulkanEngine::init_background_pipelines()
{
    ...
    // add the background effects in the array
...
void VulkanEngine::init_imgui()
{
    ...
    // the size of the pool is very oversized, but its copied from the imgui demo
    ...
    // dynamic rendering pipeline for imgui to use
...
void VulkanEngine::draw()
{
    ...
    // drawimgui into the swapchain image
...
GPUMeshBuffers VulkanEngine::uploadMesh(std::span<uint32_t> indices, std::span<Vertex> vertices)
{
    ...
    // since buffer is GPU only, create a CPU writeable staging buffer on the 
    // gpu that can copy over to the GPU only buffer
